...
<template>
  <mars-dialog :draggable="false" custom-class="query-poi-pannel" top="10" left="10">
    <div class="query-poi" @mousedown="clickVoid">
      <div class="query-poi__search">
        ...
        <mars-button class="button">
          <mars-icon icon="search" width="20" color="#fff" @click="selectPoint(searchTxt)" />
        </mars-button>
      </div>

      <ul v-if="searchListShow" class="search-list">
          ...
          <ul>
            <li v-for="(item, i) in siteSource" :key="i" class="query-site__item" @click.stop="flyTo(item)">
              <div class="query-site__context">
                ...
                <p class="query-site-sub f-toe">
                  {{ item.type }}
                </p>
              </div>
              <a :href="url + item.id" target="_blank" class="query-site__more">
                <mars-icon icon="double-right" width="20" />
              </a>
            </li>
          </ul>
          <div class="query-site__page">
            <p class="query-site-allcount">
              共{{ allCount }}条结果
            </p>
            <a-pagination size="small" :total="allCount" :page-size="6" :simple="true" @change="(page: number) => querySiteList(searchTxt, page)" />
          </div>
        </template>
        <a-empty v-else class="f-push-10-t" />
      </div>
    </div>
  </mars-dialog>
</template>
