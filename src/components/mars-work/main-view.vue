  ...
  <ConfigProvider :locale="locale">
    <div id="mars-main-view" class="mars-main-view">
      <div id="centerDiv" class="centerDiv-container">
        <MarsMap :url="configUrl" :options="mapOptions" @onload="marsOnload" />
      </div>
      <template v-if="loaded">
        <template v-for="comp in widgets" :key="comp.key">
          <MarsWidget v-if="openAtStart.includes(comp.name) && comp.visible" v-model:visible="comp.visible" :widget="comp" />
        </template>
      </template>
    </div>
  </ConfigProvider>
