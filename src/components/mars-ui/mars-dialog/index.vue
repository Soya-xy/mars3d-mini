...
<template>
  <teleport :to="mergeProps.warpper">
    <div v-show="isFold" ref="thumbnailRef" class="mars-dialog-thumb" @click="toogleFold(false)">
    ...
    >
      <div v-if="showHeader" class="mars-dialog__header" :style="{ cursor: mergeProps.draggable ? 'move' : 'auto' }" @mousedown="dragStart">
        <mars-icon v-if="mergeProps.icon" :icon="mergeProps.icon" :width="18" color="#41A8FF" class="icon" />
        <slot v-if="slots.title" name="title" />
        <span v-else class="title">{{ mergeProps.title }}</span>
        <mars-icon v-if="mergeProps.closeable" icon="close" :width="18" class="close-btn" @click="close" />
      </div>
      <mars-icon v-else-if="mergeProps.closeable" icon="close-one" :width="18" class="close-btn__flot" @click="close" />
